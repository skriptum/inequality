...
df_simple <- read_csv("../data/DWA_ECB_simplified.csv")
```
...
p2 <- df_ratio %>%
  filter(REF_AREA %in% c("DE", "FR", "IT", "ES", "NL", "BE", "GR", "HU")) %>%
...
p5 <- df_house %>%
  filter(
    REF_AREA %in% c("DE", "FR", "IT", "ES", "NL", "BE", "GR", "HU"), #selected
    # REF_AREA %in% c("AT", "BE", "DE", "FR", "LU", "NL", "FI", "IE"), # northern
...
ggsave(p5, filename = "../output/house_prices.png", width = 8, height = 5, dpi = 500)
p5
```
